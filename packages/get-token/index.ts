import { Variable, VariableValGeneric, isVariable } from "@kane/web";
import { getTokens } from "@kane/web";
...
export const stepTokenUpOrDown = (
	...
	if (shift) {
		if (
			current === "$true" ||
			(isVariable(current) && current.name === "true")
		) {
			shift += shift > 0 ? 1 : -1;
		}
	}
	...
	const result =
		(typeof found === "string" ? tokens[found] : found) || tokens["$true"];
