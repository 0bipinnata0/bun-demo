import {
	...
	objectEntries,
	objectFromEntries,
} from "@kane/create-theme";
import type { Narrow } from "@kane/web";
...
type GetParentTheme<
	P,
	Themes extends ThemeDefinitions | undefined,
> = P extends string
...
export class ThemeBuilder<State extends ThemeBuilderState> {
	...
	>(
		...
		const incomingThemeNames = Object.keys(
		...
		const childThemes = Object.fromEntries(
			namesWithDefinitions,
		) as any as ChildThemes;
	...
	build(): ThemeBuilderBuildResult<State> {
		...
		for (const themeName in this.state.themes) {
			...
			if ("theme" in themeDefinition) {
				...
				if (!template) {
					throw new Error(
						`No template for theme ${themeName}: ${templateName}`,
					);
				}

				out[themeName] = createTheme(
					palette,
					template,
					options,
					themeName,
