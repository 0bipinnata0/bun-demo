import type { StyleObject } from "@kane/helpers";
...
export type { MediaStyleObject, StyleObject } from "@kane/helpers";
...
type NonSubThemeNames<A extends string | number> =
	A extends `${string}_${string}` ? never : A;
type BaseThemeDefinitions = TamaguiConfig["themes"][NonSubThemeNames<
	keyof TamaguiConfig["themes"]
>];
type GenericThemeDefinition =
	TamaguiConfig["themes"][keyof TamaguiConfig["themes"]];
...
type AllowedStyleValuesSettingSize =
...
export type BaseStyleProps = {
	[Key in keyof TextStylePropsBase]?:
		| TextStyleProps[Key]
		| GetThemeValueForKey<Key>;
} & {
	[Key in keyof StackStylePropsBase]?:
		| StackStyleProps[Key]
		| GetThemeValueForKey<Key>;
};
...
export type GetAnimationKeys<A extends GenericTamaguiConfig> =
	keyof A["animations"];
...
export type ThemeValueFallback =
	...
	| (TamaguiSettings["allowedStyleValues"] extends undefined
			? UnionableString
			: never)
	| Variable;
...
export type ThemeValueByCategory<K extends string | number | symbol> =
	K extends "theme"
		? ThemeTokens
		: K extends "size"
		  ? SizeTokens
...
export type ThemeValueGet<K extends string | number | symbol> =
	K extends "theme"
		? ThemeTokens
		: K extends SizeKeys
...
//
type WithThemeAndShorthands<A extends object> = WithThemeValues<
	OmitLonghands<A>
> &
...
export type TextPropsBase = TextNonStyleProps &
	WithThemeAndShorthands<TextStylePropsBase>;

export type TextStyleProps =
	WithThemeShorthandsPseudosMediaAnimation<TextStylePropsBase>;
...
export type TamaguiProviderProps = Partial<
...
type StaticConfigBase = StaticConfigPublic & {
	Component?: FunctionComponent<any> &
		StaticComponentObject<any, any, any, any, any>;
...
export type GetStyledVariants<A extends TamaguiComponent> =
	A extends TamaguiComponent<any, any, any, infer Variants> ? Variants : never;

export type GetBaseProps<A extends StylableComponent> =
	A extends TamaguiComponent<any, any, infer BaseProps> ? BaseProps : never;
...
export type FontSizeVariantSpreadFunction<A extends PropLike> =
	VariantSpreadFunction<A, FontSizeTokens>;
export type SizeVariantSpreadFunction<A extends PropLike> =
	VariantSpreadFunction<A, SizeTokens>;
export type SpaceVariantSpreadFunction<A extends PropLike> =
	VariantSpreadFunction<A, SpaceTokens>;
export type ColorVariantSpreadFunction<A extends PropLike> =
	VariantSpreadFunction<A, ColorTokens>;
...
export type FontStyleVariantSpreadFunction<A extends PropLike> =
	VariantSpreadFunction<A, FontStyleTokens>;
export type FontTransformVariantSpreadFunction<A extends PropLike> =
	VariantSpreadFunction<A, FontTransformTokens>;
export type ZIndexVariantSpreadFunction<A extends PropLike> =
	VariantSpreadFunction<A, ZIndexTokens>;
export type RadiusVariantSpreadFunction<A extends PropLike> =
	VariantSpreadFunction<A, RadiusTokens>;
export type ThemeVariantSpreadFunction<A extends PropLike> =
	VariantSpreadFunction<A, ThemeTokens>;
