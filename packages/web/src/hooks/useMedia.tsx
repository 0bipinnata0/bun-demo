import { isWeb, useIsomorphicLayoutEffect } from "@kane/constants";
...
export function useMedia(
	...
	const disableSSR = componentContext
		? getDisableSSR(componentContext)
		: useDisableSSR();
	...
	const state = useSyncExternalStore<MediaQueryState>(
		...
		() => {
			...
			const testKeys =
				componentState?.keys ??
				((!componentState || componentState.enabled) && touched
					? [...touched]
					: null);
...
export const getMediaImportanceIfMoreImportant = (
	...
	return !importancesUsed[key] || importance > importancesUsed[key]
		? importance
		: null;
};
...
export function mediaObjectToString(
...
export function mediaKeyToQuery(key: string) {
	return (
		...
		mediaObjectToString(mediaQueryConfig[key], key)
	);
}
