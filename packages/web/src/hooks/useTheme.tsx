import { isClient, isIos, isServer } from "@kane/constants";
	...
	const changedThemeState = useChangeThemeEffect(
		...
		!isServer
			? () => {
					const next =
						props.shouldUpdate?.() ??
						(keys.current.length > 0 ? true : undefined);
									...
									if (process.env.EXPO_PUBLIC_TAMAGUI_TARGET === "native") {
										...
										if (
											...
											if (scheme) {
												...
												const oppositeVal = getVariable(
													oppositeTheme?.[keyString],
												);
												if (oppositeVal) {
													const dynamicVal = {
														dynamic: {
...
export const useChangeThemeEffect = (
	...
	const [themeState, setThemeState] =
		useState<ChangedThemeResponse>(createState);
	...
	function getShouldUpdateTheme(
		...
		if (
			forceUpdate !== true &&
			!manager.getStateShouldChange(next, prevState)
		) {
			return;
		}
	...
	if (!isServer) {
		...
		useEffect(() => {
			...
			const selfListenerDispose = themeManager.onChangeTheme(
				(_a, _b, forced) => {
					if (forced) {
	...
	function createState(
		...
		if (process.env.NODE_ENV === "development" && props["debug"] && isClient) {
			console.groupCollapsed(
				` 🔷 ${themeManager.id} useChangeThemeEffect createState`,
			);
