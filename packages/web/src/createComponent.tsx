import { useComposedRefs } from "@kane/compose-refs";
import { isClient, isServer, isWeb } from "@kane/constants";
import { composeEventHandlers, validStyles } from "@kane/helpers";
		...
		startVisualizer = () => {
			...
			if (devVisualizerConfig) {
				...
				const options = {
					key: "Alt",
					delay: 800,
					...(typeof devVisualizerConfig === "object"
						? devVisualizerConfig
						: {}),
				};
...
>(staticConfig: StaticConfig) {
	...
	const component = forwardRef<Ref, ComponentPropTypes>(
		(propsIn, forwardedRef) => {
			...
			const useAnimations = animationsConfig?.useAnimations as
				| UseAnimationHook
				| undefined;
			...
			const hasTextAncestor = !!(isWeb && isText
			...
			AnimatedText = animationsConfig
				? animationsConfig.Text
				: BaseTextComponent;
			AnimatedView = animationsConfig
				? animationsConfig.View
				: BaseViewComponent;
			...
			const isAnimatedReactNative =
				hasAnimationProp && animationsConfig?.isReactNative;
			const isReactNative = Boolean(
				staticConfig.isReactNative || isAnimatedReactNative,
			);
			...
			if (process.env.NODE_ENV === "development") {
				if (debugProp && debugProp !== "profile") {
					...
					const type = isAnimatedReactNative
						? "(animated)"
						: isReactNative
						  ? "(rnw)"
						  : "";
			...
			const isMediaArray =
				splitStyles.hasMedia && Array.isArray(splitStyles.hasMedia);
			const shouldListenForMedia =
				didGetVariableValue() ||
				isMediaArray ||
				(noClassNames && splitStyles.hasMedia === true);
			const mediaListeningKeys = isMediaArray
				? (splitStyles.hasMedia as any)
				: null;
			...
			const runtimePressStyle =
				!disabled && noClassNames && pseudos?.pressStyle;
			const runtimeFocusStyle =
				!disabled && noClassNames && pseudos?.focusStyle;
			...
			const runtimeHoverStyle =
				!disabled && noClassNames && pseudos?.hoverStyle;
			const needsHoverState = runtimeHoverStyle || onHoverIn || onHoverOut;
			const isHoverable =
				isWeb &&
				!!(groupName || needsHoverState || onMouseEnter || onMouseLeave);
			...
			if (process.env.EXPO_PUBLIC_TAMAGUI_TARGET === "web") {
				if (isReactNative) {
					content = (
						<span
							className={`${
								isAnimatedReactNativeWeb ? className : ""
							} _dsp_contents`}
			...
			if (process.env.NODE_ENV === "development") {
				if (debugProp && debugProp !== "profile") {
					const element =
						typeof elementType === "string" ? elementType : "Component";
					console.groupCollapsed(
						`render <${element} /> (${internalID}) with props`,
					);
					try {
