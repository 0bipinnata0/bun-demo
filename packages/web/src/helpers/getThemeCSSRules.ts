import { simpleHash } from "@kane/helpers";
...
export function getThemeCSSRules(props: {
	...
	if (process.env.TAMAGUI_TARGET === "native") {
		...
		if (hasDarkLight) {
			...
			for (const subName of names) {
				...
				for (let depth = 0; depth < numSelectors; depth++) {
					...
					selectorsSet.add(
						`${parentSelectors.join(" ")} ${nextChildSelector}`.trim(),
					);
					selectorsSet.add(
						`${parentSelectors.join(
							" ",
						)} ${nextChildSelector}.is_inversed`.trim(),
					);
				}
			}
		}
		...
		const selectorsString = selectors
			.map((x) => {
				const rootSep =
					isBaseTheme(x) && config.themeClassNameOnRoot ? "" : " ";
			...
			if (rules) {
				const styles = Object.entries(rules)
					.flatMap(([k, v]) =>
						v
							? `${
									k === "backgroundColor" ? "background" : k
							  }:${variableToString(v)}`
							: [],
					)
					.join(";");
