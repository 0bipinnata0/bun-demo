import { isAndroid } from "@kane/constants";
import { tokenCategories } from "@kane/helpers";
...
export const propMapper: PropMapper = (
	key,
	value,
	styleStateIn,
	subPropsIn,
...
const resolveVariants: StyleResolver = (
	...
	if (isObj(variantValue)) {
		const fontFamilyUpdate =
			...
			variantValue[conf.inverseShorthands.fontFamily];

		if (fontFamilyUpdate) {
			fontFamilyResult = getFontFamilyFromNameOrVariable(
				fontFamilyUpdate,
				conf,
			);
...
export function getFontFamilyFromNameOrVariable(
	input: any,
...
const tokenCats = ["size", "color", "radius", "space", "zIndex"].map(
	(name) => ({
		name,
		spreadName: `...${name}`,
	}),
...
export const getTokenForKey = (
	...
	if (theme && value in theme) {
		...
		if (
			...
			console.info(
				` - resolving ${key} to theme value ${value}: ${valOrVar?.get?.()}`,
			);
		}
		hasSet = true;
	} else {
