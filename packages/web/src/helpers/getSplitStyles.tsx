...
export const getSplitStyles: StyleSplitter = (
	...
	for (const keyOg in props) {
		...
		if (process.env.TAMAGUI_TARGET === "web") {
			if (!styleProps.noExpand) {
				...
				if (isReactNative) {
					...
					if (
						keyInit in accessibilityDirectMap ||
		...
		let isMedia: IsMediaType =
			!isStyleLikeKey && !isPseudo && isMediaKey(keyInit);
		...
		const shouldPassProp =
			...
			(isHOC &&
				...
				keyInit in parentStaticConfig.variants) ||
		...
		const avoidPropMap =
			isMediaOrPseudo || (!isVariant && !isValidStyleKeyInit);
		...
		for (const [key, val] of expanded) {
			...
			if (
				inlineProps?.has(key) ||
				(IS_STATIC && inlineWhenUnflattened?.has(key))
			) {
			...
			if (isPseudo) {
				...
				const descriptor =
					pseudoDescriptors[key as keyof typeof pseudoDescriptors];
				...
				if (shouldDoClasses && !isEnter && !isExit) {
					const pseudoStyles = generateAtomicStyles(
						pseudoStyleObject,
						descriptor,
					);
			...
			// media
				...
				if (shouldDoClasses) {
					...
					if (isThemeMedia) {
						...
						if (
							!(
								themeName === mediaThemeName ||
								themeName.startsWith(mediaThemeName)
							)
						) {
							continue;
						}
						...
						if (!groupContext) {
							if (process.env.NODE_ENV === "development" && debug) {
								console.warn(
									`No parent with group prop, skipping styles: ${groupName}`,
								);
							}
							continue;
						}
	...
	if (process.env.TAMAGUI_TARGET === "native") {
		if (style.fontFamily) {
			...
			if (faceInfo) {
				const overrideFace =
					faceInfo[style.fontWeight as string]?.[style.fontStyle || "normal"]
						?.val;
				if (overrideFace) {
...
function mergeStyle(
	...
	if (isWeb && val?.[0] === "_") {
		...
		const shouldNormalize =
			isWeb && !disableNormalize && !styleProps.noNormalize;
...
export const getSubStyle = (
	...
	for (let key in styleIn) {
		...
		if (
			!expanded ||
			(!staticConfig.isHOC && key in skipProps && !styleProps.noSkip)
		) {
			continue;
		}
...
function mergeStylePropIntoStyle(
	styleState: GetStyleState,
	cur: Object[] | Object,
) {
	if (!cur) return;
