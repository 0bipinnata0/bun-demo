...
import { StyleObject, simpleHash } from "@kane/helpers";
...
function createDeclarationBlock(style: [string, any][], important = false) {
	...
	for (const [key, value] of style) {
		next += `${hyphenateStyleName(key)}:${value}${
			important ? " !important" : ""
		};`;
	}
	return `{${next}}`;
...
const pseudoSelectorPrefixes = (() => {
	...
	for (const key in pseudoDescriptors) {
		...
		res[pseudo.name] = `${[...Array(pseudo.priority)]
			.map(() => ":root")
			.join("")} `;
	}
	return res;
...
function createAtomicRules(
	...
	switch (property) {
		...
		case "backgroundClip":
		case "userSelect": {
			const propertyCapitalized = `${property[0].toUpperCase()}${property.slice(
				1,
			)}`;
